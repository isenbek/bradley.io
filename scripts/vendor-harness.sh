...
# duckdb-browser.mjs bare-imports apache-arrow (expects a bundler), so we
# bundle it into a self-contained ESM here; the worker is already standalone.
bun build "$DD/duckdb-browser.mjs" --format=esm --minify --outfile "$OUT/duckdb/duckdb-bundled.mjs" >/dev/null
cp "$DD/duckdb-browser-eh.worker.js" "$DD/duckdb-eh.wasm" "$OUT/duckdb/"
...
- duckdb-wasm: @duckdb/duckdb-wasm@$DUCKDB_VER dist (browser mjs bun-bundled w/ apache-arrow + eh worker + eh wasm)
